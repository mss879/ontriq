...
export const StaggeredMenu = React.forwardRef<StaggeredMenuHandle, StaggeredMenuProps>(({
  ...
  const handleNavigate = useCallback((link: string, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation(); // Prevent React portal event bubbling to the Navbar container

    if (pathname === link) {
      // Just close if we are already on this page
      closeMenu();
      return;
    }
    
    // Close the menu first to play the exit animation
    closeMenu();
                        ...
                        {it.children && expandedItems.includes(it.label) && it.children.map((child, cIdx) => (
                          <li className="sm-submenu-itemWrap relative overflow-hidden leading-normal ml-6 py-1" key={child.label + cIdx}>
                            <a
                              ...
                              href={child.link}
                              onClick={(e) => handleNavigate(child.link, e)}
