...
import { Shield, Scale, Target, Users, Timer, Lightbulb } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8" role="list">
          {values.map((value, index) => (
            ...
            >
              <motion.article 
                whileHover="hover"
                className="group flex flex-col h-full p-8 rounded-[32px] border border-zinc-200 bg-zinc-50 hover:bg-white transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-orange-500/10"
              >
                <div className="flex flex-col h-full gap-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-orange-50 text-[#F75834] transition-all duration-500 group-hover:bg-[#F75834] group-hover:text-white group-hover:shadow-lg group-hover:shadow-orange-500/20">
                      <value.icon className="h-7 w-7" />
                    </div>
                  </div>
                  
                  <div className="flex flex-col flex-1 gap-3">
                    <h3 className="text-2xl font-bold text-zinc-900 min-h-[64px] flex items-start">
                      {value.title}
                    </h3>
